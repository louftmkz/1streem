...
import { TrendingUp, Music, Users, Activity, Flame, LogOut } from 'lucide-react';
...
const MusicDashboard = () => {
  ...
  useEffect(() => {
    const savedDarkMode = localStorage.getItem('1streem_darkMode');
  ...
  const toggleDarkMode = () => {
    ...
    localStorage.setItem('1streem_darkMode', JSON.stringify(newState));
  };

  // Load Spotify metrics from backend
  const loadSpotifyMetrics = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/spotify/latest-metrics');
      if (response.ok) {
  ...
  if (!isLoggedIn) {
    return (
      <div className={`min-h-screen ${isDarkMode ? 'bg-gradient-to-b from-slate-950 via-slate-900 to-slate-800' : 'bg-gradient-to-b from-slate-50 to-slate-100'} flex items-center justify-center`}>
        <div className="text-center">
          <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-6 w-20 h-20 mx-auto mb-6 flex items-center justify-center">
            <Music size={48} className="text-white" />
          </div>
          <h1 className={`text-4xl font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-slate-950'}`}>1streem</h1>
          <p className={`text-lg mb-8 max-w-md ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
            Deine Musik-Metriken von allen Streaming-Plattformen an einem Ort
          </p>
          
          <button
            onClick={handleSpotifyLogin}
            className="px-8 py-4 bg-green-600 hover:bg-green-700 text-white font-bold rounded-full transition-all duration-200 shadow-lg mb-4"
          >
            🎵 Mit Spotify verbinden
          </button>

          <p className={`text-sm ${isDarkMode ? 'text-slate-500' : 'text-slate-500'}`}>
            Deine echten Streams, Hörer & Follower automatisch täglich aktualisiert
          </p>
        ...
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-end justify-between mb-4">
            <div className="flex items-end gap-3">
              ...
              <div>
                <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-950'}`}>1streem</h1>
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg font-bold text-xs transition-all duration-200 flex items-center gap-2 ${isDarkMode ? 'bg-red-900/30 text-red-400 hover:bg-red-900/50' : 'bg-red-100 text-red-600 hover:bg-red-200'}`}
              >
                <LogOut size={16} />
                Logout
              </button>
      ...
      <div className={`border-t mt-16 ${isDarkMode ? 'border-slate-700 bg-slate-900' : 'border-slate-200 bg-white'}`}>
        <div className="max-w-7xl mx-auto px-6 py-12">
          <div className={`rounded-2xl p-8 border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-gradient-to-br from-slate-50 to-slate-100 border-slate-200'}`}>
            <h3 className={`font-bold text-lg mb-3 ${isDarkMode ? 'text-slate-100' : 'text-slate-950'}`}>Your Data Updates Daily</h3>
            <p className={`text-sm mb-4 max-w-2xl ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
              Your Spotify metrics are automatically updated every day at 00:00 UTC. We fetch your real streams, monthly listeners, and top songs directly from Spotify.
            </p>
          </div>
        </div>
      </div>
